...
public class SelfSqlite extends SQLiteOpenHelper{
	...
	public void onCreate(SQLiteDatabase sqlite) {
		...
		// 设置
		String settingsql = "create table setting(settingid integer primary key autoincrement,mainnum integer,photonum integer, hobbynum integer, titlesize integer, titlecolor integer, titlestyle integer);";
		sqlite.execSQL(settingsql);
		// 图片
		String photosql = "create table photo(photoid integer primary key autoincrement,photouri text);";
		sqlite.execSQL(photosql);
	}
	...
	public void onUpgrade(SQLiteDatabase sqlite, int arg1, int arg2) {
		...
		sqlite.execSQL("drop table if exists photo");
		onCreate(sqlite);
	}
