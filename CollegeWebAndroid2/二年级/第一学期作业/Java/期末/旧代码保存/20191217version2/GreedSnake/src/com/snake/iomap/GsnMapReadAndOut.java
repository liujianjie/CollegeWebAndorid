...
//				System.out.println(GsnMainGameConstants.gsngamemap[i].gsmsnakebean[j].snmapi[1]+" "+GsnMainGameConstants.gsngamemap[i].gsmsnakebean[j].snmapj[1]);
	...
	public String getStrByEqutor(String st){// 提取一个字符由=隔开的数据，即：level=1
		return st.substring(st.indexOf("=") + 1);
	}
	public int getStrByEqutorToInt(String st){//  提取一个字符由：隔开的数据，即：level=1 并转换为int
		return Integer.valueOf(getStrByEqutor(st));
	}
	// 读取 一个地图 从String读
...
//			System.out.println(totalCount);
	...
	public void readSetting(){
		ga.readSet();// 开始读取了地图数据
		// 开始转换
		String txtstr = ga.setsb.toString();
		if(!txtstr.trim().equals("")){
			GsnMainGameConstants.snakecomputercount = getStrByEqutorToInt(txtstr);
		}
	}
	public void outSetting(GsnSettingBean gs){
		ga.outSet(gs);
	}
