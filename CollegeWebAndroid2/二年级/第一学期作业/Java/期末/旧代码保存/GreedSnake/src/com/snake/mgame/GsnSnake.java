...
//			System.out.println("减少分数 减少蛇节 重新赋予初始位置");
	...
	public void draw(Graphics g){
		Color c = g.getColor();
		g.setColor(GsnMainGameConstants.snakeColor[playertag - 1]);// 相应的颜色
		if(GsnMainGameConstants.drawSize == 10){
			g.drawImage(GsnMainGameConstants.snakeimgresourceten[playertag - 1][direction], snactualj[0] * GsnMainGameConstants.drawSize, snactuali[0] * GsnMainGameConstants.drawSize, gsnmgjpanel);
		}else{
			g.drawImage(GsnMainGameConstants.snakeimgresourcefive[playertag - 1][direction], snactualj[0] * GsnMainGameConstants.drawSize, snactuali[0] * GsnMainGameConstants.drawSize, gsnmgjpanel);
		}
		for(int i = 1; i < node; i++){
