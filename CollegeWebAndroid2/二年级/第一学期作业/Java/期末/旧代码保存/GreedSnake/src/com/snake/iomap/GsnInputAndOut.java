...
public class GsnInputAndOut {
	public StringBuffer setsb = new StringBuffer();
...
//			System.out.println(sb.toString());
	...
	// 读取设置 
	public void readSet(){
		setsb.delete(0, setsb.toString().length());// 清楚
		File f = new File("src/com/snake/maptxt/setting.txt");
		try{
			FileReader fir = new FileReader(f);
			BufferedReader bf = new BufferedReader(fir);
			String s;
	...
	public void outSet(GsnSettingBean gs){
		File f = new File("src/com/snake/maptxt/setting.txt");
		FileWriter fir = null;
		BufferedWriter bf = null;
		try{
			fir = new FileWriter(f);// 直接覆盖
			bf = new BufferedWriter(fir);
			// 太大不知道有没有问题
			bf.write(gs.toString());
