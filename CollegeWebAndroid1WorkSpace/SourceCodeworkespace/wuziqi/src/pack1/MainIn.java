...
class MainIn extends JFrame implements Runnable, ActionListener {
  ...
  setLocationRelativeTo(null);
	...
	private void gobangAI3(int x, int y) {
		...
		if (x >= 0 && x < length && y >= 0 && y < length && grid[x][y] == check) {
			...
			if (count == step) {
				if (x + displace_x >= 0 && x + displace_x < length && y + displace_y >= 0 && y + displace_y < length
						&& grid[x + displace_x][y + displace_y] == 0) {
					if (x - step * displace_x < 0 || x - step * displace_x >= length || y - step * displace_y < 0
							|| y - step * displace_y >= length || !gobang_SpaceAI(x + displace_x, y + displace_y)
							|| (grid[x - step * displace_x][y - step * displace_y] != 0
									&& step < 3)) /* 對點狀況 */
						gobangAI3_2();
					else
	...
	private void setMark2(int x, int y) {
		...
		} while (temp_x >= 0 && temp_x < length && temp_y >= 0 && temp_y < length && grid[temp_x][temp_y] == 0
				&& space < 4);

		if (space + step >= 5)
			setMark(x, y);
		else
			setMark(x - (step + 1) * displace_x,
					y - (step + 1) * displace_y); /* 改下對點 */
	}
	...
	private void Save() {
		...
		try {
			...
			for (int i = 0; i < steps.size(); i++) {
				str = (((Grid) steps.get(i)).getLocX()) + "," + (((Grid) steps.get(i)).getLocY()) + ","
						+ (((Grid) steps.get(i)).getValue());
	...
	private void Load() {
		try {
			...
			while (read.ready()) {
				...
				grids[Integer.parseInt(step_array[0])][Integer.parseInt(step_array[1])]
						.setValue(Integer.parseInt(step_array[2]));
