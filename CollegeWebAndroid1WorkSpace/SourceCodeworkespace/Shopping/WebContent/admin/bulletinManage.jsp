...
<%	
PageModel<BulletinInfo> pm = (PageModel<BulletinInfo>)request.getAttribute("allBulletin");
if(pm== null){
	pm = new PageModel<BulletinInfo>();
}
ArrayList<BulletinInfo> items = pm.getData();
%>
...
<body>
	<form name="myForm" action="servlet/BulletinManage" method="post" onsubmit="return search_click();">
		<%
	...
	<tr>
		...
		<td><%=item.getId()%></td>
		<td class="title_td">
		<%=StringUtil.omitString(item.getTitle(),80) %>
		</td>
