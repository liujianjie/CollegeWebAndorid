  ...
  <center><h1>服务生操作界面</h1></center>
    				...
    				<tr>
		    			...
		    			<td>
		    				<c:if test="${(roomorderlist[0].state!=4) }">
			    				<c:if test="${(map.state eq 0) and !(map.meal_type eq '热菜')}"><input type="button" value="退菜" class="btn btn-danger" onclick="reOneTableDe('${map.order_detail_id }','${roomorderlist[0].order_id }')"></c:if>
	...
	function hotMeal(){
    	...
    	<c:choose>
    		...
    		<c:if test="${(roomorderlist[0].state<2)}">
...
<!-- <input type=button class="btn btn-primary"name='button_export' title='打印1' onclick="preview(1)" value="打印"> -->
 <a href="${path}/emp_SetRoom.action"style="color:white;font-size: 20px;"><button class="btn btn-info">返回</button></a>
  </body>
</html>
