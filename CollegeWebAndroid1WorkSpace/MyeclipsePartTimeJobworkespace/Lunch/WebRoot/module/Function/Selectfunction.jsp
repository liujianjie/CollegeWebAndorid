...
<html>
  <head>
	...
	<script type="text/javascript">
	  	     function del(nowpage,functionid,state){
				$.ajax({
					...
					success:function(data){
						if(data.state){
		                    ...
		                    setTimeout("window.location.href='${path}/fun_SelParentFunction.action?nowpage="+nowpage+"'",900);
						}
					}
				});
		}
	  	function updateall(functionid){
	  	   window.location.href="${path}/fun_FindFunctionByid.action?functionid="+functionid;
	  	}
	  	function addRole(){
	  	   window.location.href="${path}/fun_FindFunctionByid.action";
	  	}
	  	function goTo(nowpage){
	        window.location.href="${path}/fun_SelParentFunction.action?nowpage="+nowpage;
       }
	</script>
  </head>
  <body>
   <div id="dhead">
	    ...
	    <table  border="1" class="table table-bordered table-hover table-striped">
	     <thead>
	        ...
	        <c:if test="${!empty parentfun }">
	             <c:forEach items="${parentfun }" var="map" varStatus="aa">
	                <tbody>
	                 <tr align="center">
	                    ...
	                    <td>
	                       <c:if test="${map.state eq 1}">
									<input type="button" onclick="del(${page.nowpage },${map.function_id},0)" name="change" id="change" value="启用" class="btn btn-danger"/>
								</c:if>
								<c:if test="${map.state eq 0}">
								    <input type="button"  onclick="updateall('${map.function_id}')"
							   			value="修改" class="btn  btn btn-info " >
									<input type="button" onclick="del(${page.nowpage },${map.function_id},1)" name="change" id="change" value="禁用" class="btn btn-danger"/>
								</c:if>
	                    </td>
	                 </tr>
	                 </tbody>
