...
<html>
  <head>
	...
	<style>
		...
		.top p{background:#ffd7d7;margin-left:22px;width:700px;height:20px;margin-top:5px;margin-bottom: 5px}
		...
		<tr align="center">
			...
			<td width="120">
				<div id="topfoot" class="topxuanze">
    				<div id="wrap">
  					<div id="content" class="toptext">
						<a href="goods/qtlist?moldid=8" target="main">购物</a>
					</div>
					</div>
				</div>
			</td>
					...
					<div id="wrap">
  						<div id="content">
							<font size="2" color="red">
								...
								<c:if test="${remenlist!=null}">
									<c:forEach items="${remenlist}" var="remen">
										<a href="goods/threeqtlist?id=${remen.bottom_id }&moldid=${remen.mold_id }">${remen.bottom }</a>&nbsp;&nbsp;&nbsp;&nbsp;
									</c:forEach>
								</c:if>
							</font>
						</div>
  						...
  						<div id="content" >
							<font size="2" color="red">
								...
								<c:forEach items="${list}" var="lists">
									<a href="goods/threeqtlist?id=${lists.bottom_id }&moldid=${lists.mold_id }">${lists.bottom }</a>&nbsp;&nbsp;&nbsp;&nbsp;
								</c:forEach>
							</font>
						</div>
					</div>
    				...
    				<td colspan="2">
						...
						<!-- 一开始的商品去的是分页 -->
   							<c:if test="${zuoyoucount%2==0}">
   								<a href="goods/selstartshouyegoods?currp=${currPage+1 }"><img src="file/${goodslists.g_img }" width="340" height="263" class='curyou' ></a>
   							</c:if>
   							
   							<c:if test="${zuoyoucount%2==1}">
   								<a href="goods/selstartshouyegoods?currp=${currPage-1 }"><img src="file/${goodslists.g_img }" width="340" height="263" class='curzuo'></a>
   							</c:if>
   						...
   						<!-- 分类的商品 -->
   							<a href="goods/goodsxq?id=${goodslists.g_id }" target="_blank"><img src="file/${goodslists.g_img }" width="340" height="240"></a>		
   						</c:if>
    				</td>
    			</tr>
    			<tr height="30">
    				...
    				<c:if test="${currPage>0}">
    					<div id="yksgoodsname">
	    					<a href="goods/goodsxq?id=${goodslists.g_id }" target="_blank">
    	...
    	<!-- 分类去更多 -->
	    	<c:if test="${goodssize>0}">
	  			<div id="selmored" style="margin-top:5px;">
	  				<a href="goods/selmore?mdid=${moldid}&bottomid=${bottomid}" target="_blank">查看更多</a>
	  			</div>
	 		</c:if>
    	</li>
    </ul>
