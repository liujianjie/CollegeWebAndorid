...
//		String sql="insert into bottom values(?,?)";
		// mysql指定列名
		String sql="insert into bottom(bottom,mold_id) values(?,?)";
		try {
			PreparedStatement prst=conn.prepareStatement(sql);
	...
	public int selonnbottom_id(int mold_id) {
		...
		Connection conn=DBconnection.getconnection();
		// mysql
		String sql="select bottom_id from bottom where mold_id=? limit 0,1";
		// sql
...
//		String sql="select * from(select bottom_id from bottom where mold_id=?)where rownum<=1 ";
	...
	public BottomBean seloneBottomByMoldId(int mold_id) {
		BottomBean bt = new BottomBean();
		Connection conn=DBconnection.getconnection();
		// mysql
		String sql="select * from bottom where mold_id=? limit 0,1";
		try {
			PreparedStatement prst=conn.prepareStatement(sql);
			prst.setInt(1, mold_id);
			ResultSet rs=prst.executeQuery();
			while(rs.next()){
				bt.setBottom_id(rs.getInt("bottom_id"));
				bt.setBottom(rs.getString("bottom"));
				bt.setMold_id(rs.getInt("mold_id"));
			}
			rs.close();
			prst.close();
			conn.close();
