...
<BODY leftMargin=0 topMargin=0 marginwidth="0" marginheight="0">
	<div class="soubg">
		<div class="sou">
			<span class="s_city_b"> <span class="fl"><a
					...
					onclick="AddFavorite('http://localhost:8080/JL_GOUWU/qiantai_main/main.jsp','杰瑞购物网');">
					...
					<span class="s_city"> <span>欢迎您,亲爱的<font color="red">${jruser.uname  }</font>&nbsp;<a
							href="index?method=deljruser" target="_top">退出</a></span>
					</span>
				</c:if>
	...
	<div class="imgdivone">
		<div class="divtwo">
			<form action="goods?method=selxsgoods" target="_blank" name="frm"
				method="post">
									...
									<div id="content">
										<font size="3" color="red"> 相关分类:&nbsp;&nbsp;</font><font
											size="3"> <c:forEach items="${list}" var="lists">
												<a
													href="goods?method=selmore&mdid=${moldid}&bottomid=${lists.bottom_id}"
						...
						<c:forEach items="${goodslist}" var="list">
							<li style="width: 45%; float: left;margin-left:25px">
								<table align="center" border="0" style="table-layout: fixed">
									<tr>
										<td colspan="2"><a
											href="goods?method=goodsxq&id=${list.g_id }" target="_top"><img
					...
					<table width="68%" align="center" border=0 cellspacing="0">
						<tr align="left" height="50">
							<td><font color="#008080"> <a
									href="goods?method=selmore&mdid=${moldid}&bottomid=${bottomid}&currp=1">首页</a>
									<a
									href="goods?method=selmore&mdid=${moldid}&bottomid=${bottomid}&currp=${currPage-1 }">上一页</a>
		
									<c:forEach begin="${currPage}" end="${currPage+2}" var="i"
										step="1">
										<c:if test="${i<=totalPage}">
											<a
												href="goods?method=selmore&mdid=${moldid}&bottomid=${bottomid}&currp=${i }">${i }</a>
										</c:if>
									</c:forEach> <a
									href="goods?method=selmore&mdid=${moldid}&bottomid=${bottomid}&currp=${currPage+1}">下一页</a>
									<a
									href="goods?method=selmore&mdid=${moldid}&bottomid=${bottomid}&currp=${totalPage}">尾页</a>
	...
	<script>
		function fangfa() {
			...
			if (isNaN(page)) {
				...
				location.href = "goods?method=selmore&mdid=${moldid}&bottomid=${bottomid}&currp="
						+ page;
			}
		}
	</script>
