...
public class UserOrdersImp implements IUserOrders{
	...
	public boolean zhuce(JRuserBean jr) {
		...
		String sql="insert into jruser values(SEQ_REVIEW_JERRY.nextval,?,?,?,?,?,?,?)";
		try {
	...
	public boolean addorders(OrdersBean order) {
		Connection conn=DBconnection.getconnection();
		String sql="insert into orders values(SEQ_REVIEW_JERRY_ORDER.nextval,?,?,?,?,?,?,?,?,?,?,?)";
		try {
			PreparedStatement prst=conn.prepareStatement(sql);
...
//		String sql="select top "+pageCount+" * from orders where orders_id not in(select top "+(currPage-1)*pageCount+" orders_id from orders) and jruser_id=?";
	...
	public boolean addcomment(CommentBean co) {
		Connection conn=DBconnection.getconnection();
		String sql="insert into comments values(SEQ_REVIEW_JERRY.nextval,?,?,?,?)";
		try {
			PreparedStatement prst=conn.prepareStatement(sql);
	...
	public List selcomment(int pageCount,int currPage,int gid) {
		List list=new ArrayList();
		Connection conn=DBconnection.getconnection();
//		String sql="select top "+pageCount+" * from comments c left join jruser j on c.jruser_id=j.jruser_id where c_id not in (select top "+(currPage-1)*pageCount+" c_id from comment) and g_id=?";
