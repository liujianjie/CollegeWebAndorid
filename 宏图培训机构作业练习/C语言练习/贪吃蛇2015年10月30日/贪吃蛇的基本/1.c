...
void she(){
	...
	while(snake.life){
	   while(!kbhit()){//实现每按键输入也会运行
		...
		if(food1.x>0 &&food1.x<639 &&food1.y>234 &&food1.y<244){
			setcolor(15);
		...
		if(food.x>0 &&food.x<639 &&food.y>234 &&food.y<244){
			setcolor(15);
			rectangle(food.x,food.y,food.x+10,food.y+10);
			food.yes=0;
		}
		...
		if(snake.level==1){//等级1
			...
			if(snake.x[0]>0 &&snake.x[0]<639 &&snake.y[0]>234 &&snake.y[0]<244){
				snake.life=0;
			}
...
void tongguan(){
	...
	outtextxy(140,245,"your customs clearance");
}