...
void she(){
	...
	while(snake.life){
	   while(!kbhit()){//实现每按键输入也会运行
		...
		if(food1.x>209 &&food1.x<219 &&food1.y>309 &&food1.y<479 ){
			setcolor(15);
			rectangle(food1.x,food1.y,food1.x+10,food1.y+10);
			food1.yes=0;
			
		}
		if(food1.x>409 &&food1.x<309 &&food1.y>419 &&food1.y<479 ){
			setcolor(15);
			rectangle(food1.x,food1.y,food1.x+10,food1.y+10);
			food1.yes=0;
		...
		if(food.x>209 &&food.x<219 &&food.y>309 &&food.y<479 ){
			setcolor(15);
			rectangle(food.x,food.y,food.x+10,food.y+10);
			food.yes=0;
			
		}
		if(food.x>409 &&food.x<309 &&food.y>419 &&food.y<479 ){
			setcolor(15);
			rectangle(food.x,food.y,food.x+10,food.y+10);
			food.yes=0;
		...
		if(snake.level==1){//等级1
			...
			if(snake.x[0]>209 &&snake.x[0]<219 &&snake.y[0]>309 &&snake.y[0]<479 ){
				snake.life=0;
				
			}
			if(snake.x[0]>409 &&snake.x[0]<309 &&snake.y[0]>419 &&snake.y[0]<479 ){
				snake.life=0;
...
void shiwu(){
	if(food.yes==0){
	randomize();	
	food.x=random(560)+30;
	while(food.x%10!=0){
		food.x++;
	}
	food.y=random(370)+60;
	while(food.y%10!=0){
...
void shiwu3(){
	if(food3.yes==0){
	randomize();	
	food3.x=random(540)+50;
	while(food3.x%10!=0){
		food3.x++;
	}
	food3.y=random(360)+70;
	while(food.y%10!=0){
		food3.y++;	//防止出现不是为10的倍数的坐标
	}
	food3.yes=1;//这样食物为1 的话就不会产生新的食物
	}
	setcolor(GREEN);
	rectangle(food3.x,food3.y,food3.x+10,food3.y+10);//解决蛇会磨食物的边
}
void shiwu4(){
	int i;
	if(food4.yes==0){
