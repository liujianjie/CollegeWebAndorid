...
//		this.qqusergame=qqusergame;
		...
		int b=a*20;
		//分数 不需保存 统一死亡保存
//		if(b>t.game.getShescore()){
//			qser.addscore(t.game.getQqnum(), b);//保存分数
//		}
	...
	class B implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			if(e.getSource()==it1){
				//弹出选择框
				String message="提示："+t.game.getQqname()+"用户的以前记录会被覆盖,分数以最后死亡为主";
				int	j=JOptionPane.showConfirmDialog(null, message, "警告", JOptionPane.OK_CANCEL_OPTION);
				//保存蛇节
				if(j==0){//如果选择的是确定
					//修改蛇节 和蛇头 还有食物的坐标 这里就不要改分数了 由统一死亡才改分数
					qserive.updshenode(t.game.getQqnum(), t.node,t.x[0],t.y[0],t.x[1],t.y[1],t.foodx,t.foody,t.direction);
					//蛇头
					//System.out.println(t.x[0]+"为什么  "+t.y[0]+" "+t.x[1]+" "+t.y[1]);
