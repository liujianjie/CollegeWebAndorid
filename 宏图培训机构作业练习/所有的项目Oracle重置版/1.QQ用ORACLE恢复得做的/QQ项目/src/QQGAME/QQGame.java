...
public class QQGame extends JFrame{
	...
	public QQGame(QQgame qqusergame){
		super();
		this.qqgame=qseriver.shenode(qqusergame.getQqnum());//传给 贪吃蛇 以及 推箱子
		//设置组件样式
		try {
	...
	//	con.setLayout(null);
		...
		qname=new JLabel(qqgame.getQqname());
		qname.setBounds(350,0,65,50);
		qname.setToolTipText(qqgame.getQqname());
		p1.add(qname);
	...
	class A extends MouseAdapter{
		public void mouseClicked(MouseEvent e) {
			...
			if(e.getSource()==dzz){
				...
				String url = "F:/课外/JAVA/第三次下载html游戏/51CTO下载-html5飞机游戏/第10课飞机游戏/index.html";
				openLocalFile(url);
				
			}else if(e.getSource()==lzt){
				String url = "F:/课外/JAVA/第三次下载html游戏/51CTO下载-驴子跳游戏源码/驴子跳游戏源码/donkeyjump/donkeyjump/index.html";
				openLocalFile(url);

			}else if(e.getSource()==mota){
				String url = "F:/课外/JAVA/第三次下载html游戏/51CTO下载-html5魔塔/魔塔/mota.html";
				openLocalFile(url);
			}else if(e.getSource()==puyu){
			
			}else if(e.getSource()==shouwei){
				String url = "F:/课外/JAVA/第三次下载html游戏/51CTO下载-守卫游戏/Core/index.html";
				openLocalFile(url);
			}else if(e.getSource()==cjml){//没有..
	...
	public void openLocalFile(String fileurl){
