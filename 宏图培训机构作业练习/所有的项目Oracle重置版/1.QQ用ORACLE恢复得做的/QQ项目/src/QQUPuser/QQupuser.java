...
public class QQupuser extends JFrame{
	QQuserInterface qqinterface = new QQuserimp();
	JButton b1, b2;// 两个按钮
	
	JLabel l1, l2, l3, l4, l5, l6, l7, l8, l9, l10, l11, l12, l13, l14,xiao,close,face2;// 8个标签
...
public QQupuser(QQuser qquser,qqmain main) {
		super("");
		this.qquser=qquser;
		this.main=main;
		...
		l1 = new JLabel("修改资料", JLabel.CENTER);// 所在区域居中
		...
		/* 背景图片 */
		ImageIcon images = new ImageIcon("img/20111207100642689.jpg");
	...
	//方法设值
	public void shezhi(){
		t1.setText(qquser.getQqname());
		paw1.setText(qquser.getPwd());
		paw2.setText(qquser.getPwd());
		if(qquser.getSex().equals("男")){
			r1.setSelected(true);
		}else{
			r2.setSelected(true);
		}
		String faces=qquser.getFace();
		faces=faces.substring(faces.indexOf("/")+1,faces.indexOf("."));
		co1.setSelectedIndex(Integer.parseInt(faces));	

		co2.setSelectedItem(qquser.getDate());
		co3.setSelectedItem(qquser.getStar());
		co4.setSelectedItem(qquser.getNation());
		t2.setText(qquser.getYears());t3.setText(qquser.getMonths());t4.setText(qquser.getDprivate());
		t5.setText(qquser.getCountry());t6.setText(qquser.getProvince());t7.setText(qquser.getCity());
		te1.setText(qquser.getMark());
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == b1) {
			if (t1.getText().trim() == null || t1.getText().trim().equals("")) {
				...
				qquer提交.setQqnum(qquser.getQqnum());// qq号
				...
				qquer提交.setIpadd(main.qquser.getIpadd());
				qquer提交.setPort(main.qquser.getPort());
				boolean bl = qqinterface.upduser(qquer提交);
				//QQ游戏账号
				QQgame qqgame=new QQgame();
				//要相同
				qqgame.setQqnum(qquer提交.getQqnum());
				qqgame.setQqname(t1.getText());	
				boolean bl2=qqinterface.upgameuser(qqgame);
				
				if (bl&&bl2) {
					//修改面板的资料
					main.face.setIcon(new ImageIcon(qquer提交.getFace()));
					main.nicheng.setText(qquer提交.getQqname());
					main.text.setText(qquer提交.getMark());
					main.qquser=qquer提交;main.qqusergame=qqgame;
					JOptionPane.showMessageDialog(null,"修改成功");
					//发送消息给面板刷新
					main.notic(Contants.CMD_FLUSH);
	...
	private void setDragable() {
		this.addMouseListener(new java.awt.event.MouseAdapter() {
			public void mouseReleased(java.awt.event.MouseEvent e) {
				isDragged = false;
				QQupuser.this.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
			}
		...
		this.addMouseMotionListener(new java.awt.event.MouseMotionAdapter() {
			public void mouseDragged(java.awt.event.MouseEvent e) {
				if (isDragged) {
					loc = new Point(QQupuser.this.getLocation().x + e.getX() - tmp.x,
							QQupuser.this.getLocation().y + e.getY() - tmp.y);
					QQupuser.this.setLocation(loc);
				}
			}
		});
	}
