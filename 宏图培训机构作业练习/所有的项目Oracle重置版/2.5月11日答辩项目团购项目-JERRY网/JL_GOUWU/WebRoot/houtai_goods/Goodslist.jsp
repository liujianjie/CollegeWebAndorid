...
<html>
  ...
  <body>
  	...
  	<form action="goods?method=list" method="post">
    <table width="100%" align="center" bgcolor="black" cellspacing="1.5">
    	...
    	<c:forEach items="${list}" var="list">
    		<tr align="center" bgcolor="white"> 
					...
					<td>${list.bottom }</td>
					<td>
						<a href="goods?method=goalert&id=${list.g_id }">修改</a>
						<a href="goods?method=del&id=${list.g_id }" onclick="return confirm('确定吗？')">删除</a>
					</td>
			</tr>
    	</c:forEach>
    	<tr bgcolor="white" height="30">
    		<td colspan="10">
				<font color="#008080">
    			<a href="goods?method=list&currp=1">首页</a>		    			
    			<a href="goods?method=list&currp=${currPage-1 }">上一页</a>
				<c:forEach begin="${currPage}" end="${currPage+2}" var="i" step="1">
					<c:if test="${i<=totalPage}">
    					<a href="goods?method=list&currp=${i }">${i }</a>				    		
    				</c:if>
				</c:forEach>
		    	<a href="goods?method=list&currp=${currPage+1}">下一页</a>	
		   		<a href="goods?method=list&currp=${totalPage}">尾页</a>
	...
	<script>
	    	function fangfa(){
	    		...
	    		if(isNaN(page)){
	    			...
	    			location.href="goods?method=list&currp="+page;
	    		}
	    	}
   	</script>
  </body>
