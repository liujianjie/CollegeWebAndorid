...
<body>
	...
	<div class="item-detail wrap">
		<div class="breadcrumbs">
			<a href="<%=path%>/foregroundSystem/index.jsp">首页</a>&gt; <a href="#">债权转让详情</a>&gt;
		...
		<div class="item-detail-head clearfix" data-target="sideMenu">
			...
			<div class="bd clearfix">
				<div class="data">
					<ul>
						<li><span class="f16">转让金额</span><br> <span
									...
									<c:choose>
										<c:when test="${user.zsname!=''&&user.zsname!=null }">
											<a href="#" class="bor_btn"
												onclick="showSpan('alert-checkOldMobile');">立即购买</a>
										</c:when>
										<c:otherwise>
											<a href="#" class="bor_btn"
												onclick="alert('您没有登陆或者未绑定个人信息');">立即购买</a>
										</c:otherwise>
									</c:choose>
								</div>
							</div>
		...
		<div class="item-detail wrap">
			<div class="breadcrumbs">
				<span class="cur">该债权项目收益目标详情</span>
			</div>
			<div class="item-detail-head clearfix" data-target="sideMenu">
				<div class="hd">
					<i class="icon icon-che" title="车易贷"></i> ${oldbro.borrow_bt}
				</div>
				<div class="bd clearfix">
					<div class="data">
						<ul>
							<li><span class="f16">借款金额</span><br> <span
								class="f30 c-333" id="account">${oldbro.b_money }</span>元</li>
							<li class="relative"><span class="f16">年利率</span><br> <span
								class="f30 c-orange">${fn:substring(oldbro.rate*100,0,4)}%
							</span></li>
							<li><span class="f16">借款期限</span><br> <span
								class="f30 c-333">${oldbro.borrowqx }</span>个月</li>
							<li><span class="c-888">借款编号：</span>${fn:substring(oldbro.b_id, 1,12)}</li>
							<li><span class="c-888">发标日期：</span> <fnt:formatDate
									value="${oldbro.borrow_fbtime }" pattern="yyyy-MM-dd"></fnt:formatDate></li>
							<li><span class="c-888">剩余投标期限：</span> <c:if
									test="${oldbro.syday!=null }">
											${oldbro.syday }天 </c:if></li>
							<li><span class="c-888">还款方式：</span>${oldbro.repayment }</li>
							<li><span class="c-888">需还本息：</span>${oldbro.should_money }</li>
							<li><span class="c-888">借款用途：</span>${oldbro.borrow_yt }</li>
							<li class="colspan"><span class="c-888 fl">投标进度：</span> <br>
								<div class="progress-bar fl">
									<span style="width:${oldbro.tb_money/oldbro.b_money*100 }%"></span>
								</div> <span class="c-green">${oldbro.tb_money/oldbro.b_money*100 }%</span></li>
							<li><span class="c-888">投资范围：</span> <span
								id="account_range"> 50元~ 不限 </span></li>
						</ul>
					</div>
			...
			<div class="item-detail-body clearfix mrt30 ui-tab">
				<div class="ui-tab-nav hd">
					...
					<ul>
						<li class="nav_li active" id="nav_1"><a href="javascript:;">原借款信息</a></li>
						<li class="nav_li" id="nav_2"><a href="javascript:;">原投资记录</a>
						...
						<div class="bd">
										...
										<dd>
											<div class="text">
												<p class="MsoNormal"
													...
													借款人:&nbsp;&nbsp;${peo.pname}<span></span>
												</p>
												<p class="MsoNormal"
													style="margin-left:0cm;text-indent:0cm;">
													户籍地址为:&nbsp;&nbsp;${peo.paddress}。</p>
												<p class="MsoNormal"
													style="margin-left:0cm;text-indent:0cm;">
													性别:&nbsp;&nbsp;${peo.psex}</p>
												<p class="MsoNormal"
													...
													电话号码:&nbsp;&nbsp;${peo.ptel}</p>
												<p class="MsoNormal"
													style="margin-left:0cm;text-indent:0cm;">详细资金用途：
													&nbsp;&nbsp;${bor.borrow_yt} 。</p>
												<p class="MsoNormal"
									...
									<dl class="item">
										...
										<dd>
											<div class="verify clearfix">
												<ul>
													<c:if test="${ziliao.sfz==3 }">
														<li><i class="icon icon-4"></i><br> 身份证</li>
													</c:if>
													<c:if test="${ziliao.gzz==3 }">
														<li><i class="icon icon-7"></i><br> 工作证明</li>
													</c:if>
													<c:if test="${ziliao.srz==3 }">
														<li><i class="icon icon-4"></i><br> 学历证</li>
													</c:if>
													<c:if test="${ziliao.jzz==3 }">
														<li><i class="icon icon-5"></i><br> 户口本</li>
													</c:if>
													<c:if test="${ziliao.xyz==3 }">
							...
							<div class="ui-tab-item" style="display: none;">
								<div class="repayment-list">
									...
									<table oldbroder="0" cellpadding="0" cellspacing="0"
										width="100%">
										<tbody>
											<tr>
												<th>投标人</th>
										...
										<tbody id="repayment_content">
											<c:forEach items="${Tenders}" var="ten">
												<tr>
													...
													<td><span class="c-orange">￥
															${ten.tender_money}</span></td>
							...
							<div class="ui-tab-item" style="display: none;">
								<div class="repayment-list">
									...
									<table width="100%" oldbroder="0" cellspacing="0"
										cellpadding="0">
										<tbody>
											<tr>
												<th>合约还款日期</th>
					...
					<div class="ft-inner clearfix">
						<div class="ft-helper clearfix">
							...
							<dl>
								<dt>相关业务</dt>
								<dd>
									<a href="list.jsp">我要投资</a><a href="oldbrorow.jsp">我要借款</a>
								</dd>
							</dl>
							<dl>
								<dt>帮助中心</dt>
							...
							<dl>
								<dd>
									<p>
										...
										官方交流群:<em>12345678</em><br> 工作日 9:00-22:00 / 周六
										9:00-18:00<br>
									</p>
					...
					<div class="ft-record">
						<div class="ft-approve clearfix">
							<a class="icon-approve approve-0 fadeIn-2s" target="_blank"
								...
								target="_blank" href="#"></a><a
								class="icon-approve approve-2 fadeIn-2s" target="_blank"
								href="#"></a><a class="icon-approve approve-3 fadeIn-2s"
				...
				<div class="alert-450" id="alert-checkOldMobile"
					...
					<div class="alert-main">
						<form id="checkOldMobileForm" name="checkOldMobileForm"
							method="post" action="<%=path%>/Borrow/borrowZaiQuan.action"
							...
							<ul>
								...
								<input type="hidden" name="old_b_id" value="${oldbro.b_id}" />
								...
								<li><span class="txt-right" style="display: none;"
									id="errors"><i class="error-icon"></i><i
										class="error-tip" id="errorsmsg"></i></span><br> <input
									type="button" value="确 认" class="btn-ok txt-right"
									onclick="tijiao();"></li>
							</ul>
						</form>
					</div>
				...
				<script type="text/javascript">
					...
					function showSpan(op) {
						var tzje = $('#je').val();
						var hktz = '${oldbro.b_money-oldbro.tb_money}';
						if (isNaN(tzje)) {
							alert("请输入数字!");
