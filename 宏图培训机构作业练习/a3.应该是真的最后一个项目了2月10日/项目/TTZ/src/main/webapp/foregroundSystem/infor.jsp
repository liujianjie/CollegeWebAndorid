		...
		<div class="item-detail-head clearfix" data-target="sideMenu">
			...
			<div class="bd clearfix">
				<div class="data">
					<ul>
						...
						<li><span class="f16">借款期限</span><br> <span
							class="f30 c-333">${bor.borrowqx }</span>个月</li>
						<li><span class="c-888">借款编号：</span>${fn:substring(bor.b_id, 1,12)}</li>
						...
						<li><span class="c-888">剩余投标期限：</span> <c:if
								test="${bor.syday!=null }">
											${bor.syday }天 </c:if></li>
						<li><span class="c-888">还款方式：</span>${bor.repayment }</li>
						<li><span class="c-888">需还本息：</span>${bor.should_money }</li>
						<li><span class="c-888">借款用途：</span>${bor.borrow_yt }</li>
				...
				<c:choose>
					<c:when test="${bor.statu==2 }">
						<div class="mod-right mod-status" style="height: 240px;">
							<div class="inner">
								...
								<div class="mt15">
									预计总收益为:&nbsp;&nbsp;<font id="fontsy" size="3" color="green">0</font>元
								</div>
								<div class="mt15">
									本息为:&nbsp;&nbsp;&nbsp;&nbsp;<font id="fontbx" size="3"
										color="green">0</font>元
								</div>
								<div class="mt15">
									<c:choose>
					...
					<c:when test="${bor.statu==3 }">
						<div class="mod-right mod-status">
							<div class="inner">
								<div class="text">
									待还本息：<span class="f24 c-333">${bor.should_money-bor.repay_money }</span>元<br>
									剩余期限：<span class="f24 c-333"> <c:if
											test="${bor.syday!=null }">
										${bor.syday }天
									</c:if>
									</span> <br> 下期还款日： <span class="f20 c-333"> <fnt:formatDate
											value="${bor.repay_lasttime }" pattern="yyyy-MM-dd"></fnt:formatDate>
									</span>
								</div>
	...
	<div class="alert-450" id="alert-checkOldMobile" style="display:none;">
		...
		<div class="alert-main">
			<form id="checkOldMobileForm" name="checkOldMobileForm" method="post"
				action="<%=path%>/Borrow/borrowTZ.action"
	...
	<script type="text/javascript">
		...
		function yjsy(val) {
			if (isNaN(val)) {
				...
				//获取月利率
				var monRate = '${bor.rate}' / 12;
				var mouth = '${bor.borrowqx}'; //收益期限
				var fs = '${bor.repayment}';
				if (fs == '等额本息') {
					var monInterest = val * monRate * Math.pow((1 + monRate), mouth)
						/ (Math.pow((1 + monRate), mouth) - 1);
					var sumbx = monInterest * mouth; //本息
					$("#fontsy").html(changeTwoDecimal(sumbx - val)); //收益
					$("#fontbx").html(changeTwoDecimal(sumbx)); //本息
				} else if (fs == '等额本金') {
					var monthPri = val / mouth; //每月本金
					var sumbx = 0.00;
					for (var i = 1; i <= mouth; i++) {
						var monthRes = monthPri + (val - monthPri * (i - 1)) * monRate;
						sumbx += monthRes;
					}
					$("#fontsy").html(changeTwoDecimal(sumbx - (monthPri * mouth))); //收益
					$("#fontbx").html(changeTwoDecimal(sumbx)); //本息
				}
			}
