							...
							<tbody>
								<c:forEach items="${mblist }" var="mb">
									<tr>
										...
										<td>${mb.tname }</td>
										<td><c:choose>
												<c:when test="${mb.statu==3 }"><font color="#0bfb16">满标</font></c:when>
												<c:when test="${mb.statu==4 }"><font color="#777888">还款中</font></c:when>
										   </c:choose></td>
										<td>${mb.b_money }</td>
										<td>${mb.borrow_yt }</td>
										<td><span class="pie">${mb.borrow_type }</span></td>
										<td>${mb.repayment }</td>
										<td><fmt:formatDate value="${mb.borrow_time }"
												pattern="yyyy-MM-dd HH:mm:ss" /></td>
										<td><c:choose>
												<c:when test="${mb.statu==3 }">
													<a
														href="<%=path %>/Borrow/updMBstatu.action?statu=4&id=${mb.b_id}"
														onclick="return confirm('您确定要放款吗？');"><i
														class="fa fa-check text-navy"></i> 放款</a>
	                                   				 	&nbsp;&nbsp;
												</c:when> 
											</c:choose></td>
									</tr>
