...
<link
	...
	rel="stylesheet">
<link rel="icon"
	href="${pageContext.request.contextPath}/backgroundSystem/img/profile_small.jpg"
	type="image/x-icon">
<!-- 文件 -->
<link
	...
	<div id="wrapper">
		...
		<div class="sidebar-collapse">
			<ul class="nav" id="side-menu">
				<li class="nav-header">
					<div class="dropdown profile-element">
						<span><img alt="image" class="img-circle" id="touxiang"
							src="${pageContext.request.contextPath }/${backuser.b_touxiang}"
							style="width:70px;height:70px" /></span> <a data-toggle="dropdown"
							class="dropdown-toggle" href="index.jsp#"> <span
						...
						<ul class="dropdown-menu animated fadeInRight m-t-xs">
							...
							<li><a class="J_menuItem"
								href="${pageContext.request.contextPath }/backuser/toalert.action?id=${backuser.b_id}&v=3.0">个人资料</a></li>
					...
					<li><a href="layouts.jsp"><i class="fa fa-columns"></i> <span
						...
						<ul class="nav nav-second-level">
							...
							<c:if test="${fn:contains(qxlist,'5') }">
								<li><a class="J_menuItem"
									href="${pageContext.request.contextPath }/post/postlist.action">岗位管理</a></li>
							</c:if>
						</ul></li>
				</c:if>


				<c:if test="${fn:contains(qxlist,'6') }">
					<li><a href="index.jsp#"> <i class="fa fa fa-bar-chart-o"></i>
							<span class="nav-label">网站会员</span> <span class="fa arrow"></span>
					</a>
						<ul class="nav nav-second-level">
							<c:if test="${fn:contains(qxlist,'7') }">
								<li><a class="J_menuItem"
									href="<%=path%>/user/selectAllTtzUser.action">注册会员管理</a></li>
							</c:if>	
							<c:if test="${fn:contains(qxlist,'8') }">
								<li><a class="J_menuItem" href="">会员认证管理<span
										class="fa arrow"></span></a>
								<ul class="nav nav-third-level">
									<c:if test="${fn:contains(qxlist,'9') }">
										<li><a class="J_menuItem"
											href="${pageContext.request.contextPath}/rengzheng/selzilaio.action">基本资料认证</a>
										</li>
									</c:if>
									<c:if test="${fn:contains(qxlist,'10') }">
										<li><a class="J_menuItem"
											href="${pageContext.request.contextPath}/user/newuser.action">新用户认证资料</a>
										</li>
									</c:if>
									<c:if test="${fn:contains(qxlist,'11') }">
										<li><a class="J_menuItem"
											href="${pageContext.request.contextPath}/rengzheng/selall.action">认证资料统计</a>
										</li>
									</c:if>
								</ul></li>
							</c:if>
				...
				<c:if test="${fn:contains(qxlist,'12') }">
					<li><a href="mailbox.jsp"><i class="fa fa-envelope"></i> <span
						...
						<ul class="nav nav-second-level">
							<c:if test="${fn:contains(qxlist,'13') }">
								<li><a class="J_menuItem" href="Withdraw.jsp">提现管理</a></li>
							</c:if>
							<c:if test="${fn:contains(qxlist,'14') }">
								<li><a class="J_menuItem" href="bankcardManage.jsp">银行卡管理</a></li>
							</c:if>
							<c:if test="${fn:contains(qxlist,'15') }">
								<li><a class="J_menuItem"
									href="<%=path%>/moneyrecord/TopupMoner.action">充值记录</a></li>
							</c:if>	
							<c:if test="${fn:contains(qxlist,'16') }">
								<li><a class="J_menuItem"
									href="<%=path%>/ListFactorage.action">手续费记录</a></li>
							</c:if>	
							<c:if test="${fn:contains(qxlist,'17') }">		
								<li><a class="J_menuItem"
									href="<%=path%>/moneyrecord/TradingRecord.action">交易记录</a></li>
							</c:if>
						</ul></li>
				</c:if>

				<c:if test="${fn:contains(qxlist,'18') }">
					<li><a href="index.jsp#"><i class="fa fa-desktop"></i> <span
							class="nav-label">借贷管理</span><span class="fa arrow"></span></a>
						<ul class="nav nav-second-level">
							<c:if test="${fn:contains(qxlist,'19') }">
								<li><a class="J_menuItem" href="<%=path%>/touzhi.action">投资管理</a></li>
							</c:if>
							
							<c:if test="${fn:contains(qxlist,'20') }">
								<li><a class="J_menuItem"
									href="<%=path%>/borrow_list.action">借贷管理</a></li>
							</c:if>
						</ul></li>
				</c:if>

				<c:if test="${fn:contains(qxlist,'21') }">
					<li><a href="index.jsp#"><i class="fa fa-desktop"></i> <span
							class="nav-label">设置项</span><span class="fa arrow"></span></a>
						<ul class="nav nav-second-level">	
							<c:if test="${fn:contains(qxlist,'22') }">	
								<li><a class="J_menuItem" href="<%=path %>/rates/selrateslist.action">费率设置</a></li>
							</c:if>
							
							<c:if test="${fn:contains(qxlist,'23') }">
								<li><a class="J_menuItem" href="<%=path %>/FundraisingDeadlinelist.action">筹款期限设置</a></li>
							</c:if>
						</ul></li>
				</c:if>
				<c:if test="${fn:contains(qxlist,'24') }">
					<li><a href="index.jsp#"><i class="fa fa-desktop"></i> <span
							class="nav-label">审核事项</span><span class="fa arrow"></span></a>
						<ul class="nav nav-second-level">
							<c:if test="${fn:contains(qxlist,'25') }">
								<li><a class="J_menuItem"
									href="<%=path%>/Borrow/selBorrowAll.action">借款审核</a></li>
							</c:if>
							<c:if test="${fn:contains(qxlist,'26') }">
								<li><a class="J_menuItem"
									href="<%=path%>/Borrow/selBorrowMB.action">放款审核</a></li>
							</c:if>
						</ul></li>
				</c:if>
				<c:if test="${fn:contains(qxlist,'27') }">
					<li><a href="index.jsp#"><i class="fa fa-desktop"></i> <span
							class="nav-label">网站管理</span><span class="fa arrow"></span></a>
						<ul class="nav nav-second-level">
							<c:if test="${fn:contains(qxlist,'28') }">
								<li><a class="J_menuItem"
									href="<%=path%>/notice/selnoticelistback.action">网站公告</a></li>
							</c:if>
						</ul></li>
				</c:if>
				<c:if test="${fn:contains(qxlist,'29') }">
					<li><a href="index.jsp#"><i class="fa fa-desktop"></i> <span
							class="nav-label">团团赚ViP团</span><span class="fa arrow"></span></a>
						<ul class="nav nav-second-level">
							<c:if test="${fn:contains(qxlist,'30') }">
								<li><a class="J_menuItem" href="TtzVip.jsp">发布vip团</a></li>
							</c:if>
							<c:if test="${fn:contains(qxlist,'31') }">
							<li><a class="J_menuItem"
								href="<%=path%>/Borrow/SeeVIPTuan.action">vip团信息</a></li>
							</c:if>
						</ul></li>
				</c:if>
			</ul>
                	...
                	<form  class="form-horizontal m-t" >
						...
						<div class="form-group"> 
							...
							<div class="col-sm-2 control-label">
                    			<button id="sbt" data-dismiss="modal"  class="btn btn-primary"  onclick="UpladFile();">提交</button>
                			</div>
						</div>
					</form>
				</div>
