...
<script type="text/javascript"
	src="${pageContext.request.contextPath}/foregroundSystem/My97DatePicker/WdatePicker.js"></script>
			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
						<form method="post" action="../moneyrecord/ConditionQuery.action"
							class="pull-left mail-search">
							<div class="input-group">
								<input type="text" style="border-color: blue;"
									class="form-control input-sm" name="m_date"
								...
								<c:forEach items="${list}" var="list" varStatus="index">
									<tr>
										<td><c:forEach items="${user}" var="user">
												${list.p_userid==user.userid?user.username:''}
											</c:forEach></td>
										<td>${list.m_yxmoney }</td>
										<td>${list.m_jytype }</td>
										<td>${list.m_remark }</td>
										<td><f:formatDate value="${list.m_happentime }" /></td>
									</tr>
								</c:forEach>
