...
public class PersonalDao {
	...
	public List<PersonalBean> getPersonalDefaultList(){
		...
		per = new PersonalBean(1, "姓名", "小蓝");
		perlist.add(per);
		per = new PersonalBean(2, "学校", "新东方烹饪学校");
		perlist.add(per);
		per = new PersonalBean(3, "班级", "互联网1902");
		perlist.add(per);
		per = new PersonalBean(4, "学号", "11111");
		perlist.add(per);
		per = new PersonalBean(5, "性别", "男");
		perlist.add(per);
		per = new PersonalBean(6, "名族", "汉");
		perlist.add(per);
		per = new PersonalBean(7, "出生年月", "2001年1月10日");
		perlist.add(per);
		per = new PersonalBean(8, "家庭住址", "海南省");
		perlist.add(per);
		return perlist;
	}
