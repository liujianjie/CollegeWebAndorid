...
public class Project4 extends AppCompatActivity {
    ...
    private TestBaseApdater apdater;
    String searchResult;
        ...
        Button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                         searchResult = HpptRequest.HpptRequest(item, option);
                    }
                });
                if(!(item.trim().equals(""))){
...
//                        displayTableMusic(music.size()+1,3,music);
                        ...
                        try {
                            ...
                            jsonArraysoft = new JSONArray(searchResult.substring(searchResult.indexOf("["), searchResult.lastIndexOf("]") + 1));
//                            for (int i = 0; i < jsonArray.length(); i++) {
//                                JSONObject jsonObject = jsonArray.getJSONObject(i);
//
//                                String artistName = jsonObject.optString("artistName");
//                                Double price = jsonObject.optDouble("price");
//                                String supportedDevices = jsonObject.optString("supportedDevices");
//                                String description = jsonObject.optString("description");
//                                String genres = jsonObject.optString("genres");
//                                String screenshotUrls = jsonObject.optString("screenshotUrls");
//                                String ipadScreenshotUrls = jsonObject.optString("ipadScreenshotUrls");

//                                software.add(new SoftWare(artistName, price, supportedDevices, description, genres, screenshotUrls,ipadScreenshotUrls));
//                                software.add(new SoftWare(artistName, price));
//                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    ...
                    Toast.makeText(Project4.this,"请输入你要搜索的关键词",Toast.LENGTH_SHORT).show();
                }
            }
        });
        findView();
    ...
    public void findView(){
        ...
        apdater = new TestBaseApdater(this);
//        listview.setAdapter(apdater);
    }
    public void setListData(LinkedList data){
        //baseadapter
        Toast.makeText(Project4.this,"可以进来"+data.size(),Toast.LENGTH_SHORT).show();
